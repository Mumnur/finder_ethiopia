...
class _LostPostPageState extends State<LostPostPage> {
  ...
  Future<void> fetchFoundItems() async {
    final response = await http.get(
        Uri.parse('http://finderastu.pythonanywhere.com/thread/lost-item'));
  ...
  Future<void> fetchFoundPersons() async {
    final response = await http.get(
        Uri.parse('http://finderastu.pythonanywhere.com/thread/lost-person'));
    if (response.statusCode == 200) {
  ...
  Widget buildFoundItemTile(BuildContext context, dynamic item) {
    ...
    final fallbackImage = 'assets/jemal.jpg';
